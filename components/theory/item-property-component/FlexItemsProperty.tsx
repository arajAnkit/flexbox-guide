...
export const FlexItemProperties = () => {
  const flexItemConfigs = [
    {
      name: "Order",
      description: `The "order" property controls the visual arrangement of flex items 
      without changing the HTML structure. Items with lower order values appear first. 
      Default value is 0.`,
      code: `.item-1 { order: 1; }
.item-2 { order: 3; }
.item-3 { order: 2; }`,
      ...
      items: [
        { text: "Item 1 (order: 1)", className: "order-1" },
        { text: "Item 2 (order: 3)", className: "order-3" },
        { text: "Item 3 (order: 2)", className: "order-2" },
      ],
    },
    {
      name: "Flex Grow",
      description: `"flex-grow" allows an item to expand and occupy available space. 
      It works relative to other items (higher values take more space). Default is 0 (no growth).`,
...
.item-3 { flex-grow: 3; }`,
    ...
    {
      name: "Flex Shrink",
      description: `"flex-shrink" determines how much an item reduces in size relative to others 
      when there isn't enough space. Higher values shrink more. Default is 1.`,
...
.item-3 { flex-shrink: 2; }`,
    ...
    {
      name: "Flex Basis",
      description: `"flex-basis" sets the initial size of a flex item before it grows/shrinks. 
      Values can be fixed (px, %, rem) or flexible ("auto").`,
...
.item-3 { flex-basis: auto; }`,
    ...
    {
      name: "Flex (Shorthand)",
      description: `"flex" is a shorthand that combines "flex-grow", "flex-shrink", and "flex-basis" 
      into one declaration for cleaner code. Example: flex: 1 1 150px (grow: 1, shrink: 1, basis: 150px).`,
...
.item-3 { flex: 3 1 auto; }`,
  ...
  return (
    <div>
      <h3 className="mt-8 mb-4 text-xl font-semibold">Flex Item Properties</h3>
      <p className="text-foreground mb-4">
        Below are five essential flex item properties, each with descriptions,
        code snippets, and interactive previews to demonstrate their effects.
      </p>
